---
title: Svelte
description: Learn how to create an GrowFoundry project and build a Svelte app using AI
---

import Installation from '/snippets/sdk-installation.mdx';

Learn how to create an GrowFoundry project and build a Svelte app using AI tools like Cursor.
...
**GrowFoundry client** at `src/lib/growfoundry.ts`:

```typescript src/lib/growfoundry.ts
...
<script lang="ts">
  ...
  import { growfoundry } from '../growfoundry';
  ...
  onMount(async () => {
    try {
      const { data, error: fetchError } = await growfoundry.database
        .from('sports')
        .select();
