...
| `redirectTo` | string | No | Used for link-based email verification. The email link always opens an GrowFoundry backend endpoint first; after the token is verified, GrowFoundry redirects the browser to this URL with the verification result. Required when `verifyEmailMethod` is `link`. This URL must be included in `allowedRedirectUrls`. Recommended: use your app's sign-in page. |

### Example (Web Client)

```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/users" \
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/users?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "securepassword123",
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/sessions" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "securepassword123"
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/sessions?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "securepassword123"
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/refresh" \
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/refresh?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "refreshToken": "eyJhbGciOiJIUzI1NiIs..."
  }'
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/logout"
```
...
```bash
curl "https://your-app.growfoundry.app/api/auth/sessions/current" \
...
```bash
curl -X PATCH "https://your-app.growfoundry.app/api/auth/profiles/current" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
    "profile": {
...
```bash
curl "https://your-app.growfoundry.app/api/auth/profiles/123e4567-e89b-12d3-a456-426614174000"
```
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/email/send-verification" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "redirectTo": "http://localhost:3000/sign-in"
...
- Success: `?growfoundry_status=success&growfoundry_type=verify_email`
- Error: `?growfoundry_status=error&growfoundry_type=verify_email&growfoundry_error=...`
- `growfoundry_status`: Result of the browser link flow. For verification, values are `success` or `error`.
- `growfoundry_type`: Flow identifier. For verification links this is always `verify_email`.
- `growfoundry_error`: Present only when `growfoundry_status=error`. Human-readable error message for display or logging.
- Recommended handling: use your sign-in page as `redirectTo`. When `growfoundry_status=success`, show a confirmation message and ask the user to sign in with their email and password.
- If `redirectTo` is not allowlisted, GrowFoundry returns a `400` error whose message includes the rejected URL and whose `nextActions` tells you to add it to `allowedRedirectUrls`.

### Example (Web Client)

```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/email/verify" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "otp": "123456"
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/email/verify?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "otp": "123456"
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/email/exchange-reset-password-token" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "code": "123456"
...
- Ready to reset: `?token=...&growfoundry_status=ready&growfoundry_type=reset_password`
- Error: `?growfoundry_status=error&growfoundry_type=reset_password&growfoundry_error=...`
- `token`: Present only when `growfoundry_status=ready`. Pass this value to `POST /api/auth/email/reset-password` as `otp`.
- `growfoundry_status`: Result of the browser link flow. For reset links, values are `ready` or `error`.
- `growfoundry_type`: Flow identifier. For reset links this is always `reset_password`.
- `growfoundry_error`: Present only when `growfoundry_status=error`. Human-readable error message for display or logging.
- Your app should only render the reset-password form when `growfoundry_status=ready` and `token` is present.
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/email/reset-password" \
  -H "Content-Type: application/json" \
  -d '{
...
<Note>
  ...
  `response_type`, or `scope`; GrowFoundry/provider-generated values win and colliding client
  values are ignored.
</Note>
...
curl "https://your-app.growfoundry.app/api/auth/oauth/google?redirect_uri=https://myapp.com/callback&code_challenge=$CODE_CHALLENGE&prompt=select_account"
```

```bash
# Custom provider example
curl "https://your-app.growfoundry.app/api/auth/oauth/custom/acme?redirect_uri=https://myapp.com/callback&code_challenge=$CODE_CHALLENGE"
```
...
```
https://myapp.com/callback?growfoundry_code=abc123...
```

<Note>
The `growfoundry_code` is a temporary authorization code that must be exchanged for tokens using the `/api/auth/oauth/exchange` endpoint.
</Note>
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `code` | string | Yes | The `growfoundry_code` received in the callback |
| `code_verifier` | string | Yes | The original code_verifier used to generate the code_challenge |

### Example

```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/oauth/exchange?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "code": "abc123...",
...
let authURL = "https://your-app.growfoundry.app/api/auth/oauth/google" +
...
// 4. Handle callback with growfoundry_code
// myapp://callback?growfoundry_code=abc123...

// 5. Exchange code for tokens
let response = POST("/api/auth/oauth/exchange?client_type=mobile", body: {
    "code": growfoundryCode,
...
```bash
curl "https://your-app.growfoundry.app/api/auth/public-config"
```
...
```bash
curl -X DELETE "https://your-app.growfoundry.app/api/auth/users" \
...
```bash
curl "https://your-app.growfoundry.app/api/auth/config" \
  -H "Authorization: Bearer admin-jwt-token"
```
...
- If `allowedRedirectUrls` is empty, GrowFoundry allows all redirects for developer convenience. This is insecure for production and should be avoided outside local development.
...
```bash
curl -X PUT "https://your-app.growfoundry.app/api/auth/config" \
  -H "Authorization: Bearer admin-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
    "requireEmailVerification": true,
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `code` | string | Yes | Authorization code or JWT from Growfoundry Cloud |

#### Example

```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/admin/sessions/exchange" \
  -H "Content-Type: application/json" \
...
`growfoundry_admin_refresh_token` httpOnly cookie and does not share the app/user
refresh cookie.
...
```bash
curl -X POST "https://your-app.growfoundry.app/api/auth/admin/refresh" \
  -H "X-CSRF-Token: abc123..." \
  --cookie "growfoundry_admin_refresh_token=..."
```
