...
description: Direct OpenRouter HTTP integration and GrowFoundry Model Gateway helper endpoints
---

## Overview

New AI integrations should call OpenRouter directly with the OpenRouter key provisioned by GrowFoundry. The GrowFoundry AI proxy routes for chat, image generation, and embeddings are deprecated compatibility endpoints.
...
## GrowFoundry Admin Helper Endpoints
...
Use either an admin JWT bearer token or an GrowFoundry API key bearer token in the `Authorization` header.
...
```bash
curl "https://your-app.growfoundry.app/api/ai/models" \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>"
```

```bash
curl "https://your-app.growfoundry.app/api/ai/models" \
  -H "Authorization: Bearer <GROWFOUNDRY_API_KEY>"
```
...
```bash
curl "https://your-app.growfoundry.app/api/ai/openrouter/api-key" \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>"
```
...
```bash
curl "https://your-app.growfoundry.app/api/ai/overview" \
  -H "Authorization: Bearer <ADMIN_JWT_TOKEN>"
```
...
## Deprecated GrowFoundry Proxy Routes
...
This route validates an GrowFoundry-shaped request, forwards it to OpenRouter, and returns an GrowFoundry-shaped response. It does not expose the complete OpenRouter API surface.
...
This route forwards an GrowFoundry-shaped image request to OpenRouter chat completions with image output. It is retained for compatibility only.
...
This route forwards an GrowFoundry-shaped embeddings request to OpenRouter. Prefer the direct OpenRouter embeddings endpoint for new code.
