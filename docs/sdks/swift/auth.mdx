...
description: User authentication and profile management with the GrowFoundry Swift SDK
---
...
func handleSignUp(email: String, password: String, name: String?) async {
    do {
        let result = try await growfoundry.auth.signUp(
            email: email,
            password: password,
            name: name
        )
...
func handleVerifyEmail(email: String, code: String) async {
    do {
        try await growfoundry.auth.verifyEmail(email: email, code: code)
...
func handleResendCode(email: String) async {
    do {
        try await growfoundry.auth.resendVerificationEmail(email: email)
...
For users who register with email, the GrowFoundry backend provides three options:
...
3. **Code-based verification** - The GrowFoundry backend sends a 6-digit verification code to the user's email. The client app needs to display a verification screen where users can enter the code, then call `verifyEmail(email:code:)` to complete verification. Only after this can users sign in with email + password.
...
```swift
do {
    let result = try await growfoundry.auth.signIn(
...
```swift
do {
    try await growfoundry.auth.signOut()
...
```swift
do {
    if let user = try await growfoundry.auth.getCurrentUser() {
...
```swift
do {
    let result = try await growfoundry.auth.setProfile([
        "name": "JohnDev",
        "bio": "iOS Developer",
...
GrowFoundry supports two password reset methods, configured in the backend:
...
```swift
do {
    try await growfoundry.auth.sendPasswordReset(email: "user@example.com")
...
```swift
do {
    let response = try await growfoundry.auth.exchangeResetPasswordToken(
        email: "user@example.com",
        code: "123456"
    )
...
```swift
do {
    try await growfoundry.auth.resetPassword(
        otp: resetToken,
...
- `redirectTo` (`String`) - Callback URL where GrowFoundry will redirect after authentication
...
2. SDK fetches the OAuth authorization URL from GrowFoundry
...
} catch let error as GrowFoundryAuthError {
    switch error {
