...
description: File upload, download, and management with the GrowFoundry Swift SDK
---
...
```swift
let bucket = growfoundry.storage.from("images")
```
...
try await growfoundry.storage.createBucket("avatars")

// Create a private bucket
try await growfoundry.storage.createBucket(
    "documents",
...
```swift
let buckets = try await growfoundry.storage.listBuckets()
...
```swift
try await growfoundry.storage.updateBucket(
    "documents",
...
```swift
try await growfoundry.storage.deleteBucket("old-bucket")
```
...
let file = try await growfoundry.storage
    .from("images")
    .upload(
...
```swift
// Download file
let data = try await growfoundry.storage
    .from("images")
...
```swift
// List all files
let files = try await growfoundry.storage
    .from("images")
    .list()

// List with prefix filter
let userFiles = try await growfoundry.storage
    .from("images")
...
if let post = posts.first, let imageKey = post.imageKey {
    ...
    let _: [Post] = try await growfoundry.database
        .from("posts")
        .eq("id", value: "post-123")
...
```swift
let strategy = try await growfoundry.storage
    .from("videos")
    .getUploadStrategy(
