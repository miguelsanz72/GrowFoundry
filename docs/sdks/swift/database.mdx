...
description: Type-safe database operations using the GrowFoundry Swift SDK
---
...
let inserted = try await growfoundry.database
    .from("posts")
    .insert(post)
...
// Update by ID
let updated: [Post] = try await growfoundry.database
    .from("posts")
    .eq("id", value: postId)
...
let updated: [Task] = try await growfoundry.database
    .from("tasks")
...
```swift
// Delete by ID
try await growfoundry.database
    .from("posts")
    .eq("id", value: postId)
    .delete()
...
```swift
// Get all posts
let posts: [Post] = try await growfoundry.database
    .from("posts")
    .select()
    .execute()

// Filter and sort
let recentPosts: [Post] = try await growfoundry.database
    .from("posts")
    .select()
...
// Multiple filters
let activePosts: [Post] = try await growfoundry.database
    .from("posts")
    .select()
    .eq("userId", value: currentUserId)
...
// Specific columns
let posts = try await growfoundry.database
    .from("posts")
...
let stats: [UserStats] = try await growfoundry.database
    .rpc("get_user_stats", args: ["user_id": "123"])
    .execute()

// Call function with parameters, returning single result
let stat: UserStats = try await growfoundry.database
    .rpc("get_user_stats", args: ["user_id": "123"])
    .executeSingle()

// Call function without parameters
let users: [User] = try await growfoundry.database
    .rpc("get_all_active_users")
    .execute()

// Call function with multiple parameters
let results: [Post] = try await growfoundry.database
...
let count: Int = try await growfoundry.database
...
let products: [Product] = try await growfoundry.database
    .from("products")
    .select()
...
let tasks: [Task] = try await growfoundry.database
    .from("tasks")
    .select()
