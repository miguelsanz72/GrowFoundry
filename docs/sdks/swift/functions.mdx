...
description: Invoke serverless functions with the GrowFoundry Swift SDK
---
...
let response: HelloResponse = try await growfoundry.functions.invoke(
    "hello-world",
...
let response: GreetingResponse = try await growfoundry.functions.invoke(
    "hello-world",
    body: request
)
...
```swift
do {
    let response: MyResponse = try await growfoundry.functions.invoke(
        "my-function",
        body: ["key": "value"]
    )
    print("Success: \(response)")
} catch let error as GrowFoundryError {
    switch error {
