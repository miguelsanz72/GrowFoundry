...
description: User authentication and profile management with the GrowFoundry Kotlin SDK
---
...
class AuthViewModel : ViewModel() {
    ...
    suspend fun signUp(email: String, password: String, name: String?) {
        ...
        } catch (e: GrowfoundryHttpException) {
...
- `redirectUri` (`String`) - Callback URL where GrowFoundry will redirect after authentication
...
When creating the GrowFoundry client, configure the `browserLauncher` to handle opening URLs:

```kotlin
val client = createGrowfoundryClient(baseURL, anonKey) {
    install(Auth) {
...
class AuthCallbackActivity : AppCompatActivity() {
    // Example: read the shared GrowFoundry client from your Application class.
    private val client by lazy { (application as MyApp).growfoundryClient }
...
`client` should come from the same shared GrowFoundry client instance you use elsewhere in the app, such as an `Application` singleton or your DI container.
...
```kotlin
import dev.growfoundry.exceptions.GrowfoundryHttpException
import dev.growfoundry.exceptions.GrowfoundryException
