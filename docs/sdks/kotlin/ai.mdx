---
title: Model Gateway
description: Build AI features from Kotlin with the OpenRouter key provisioned by GrowFoundry
---

## Overview

GrowFoundry provisions an OpenRouter API key for Model Gateway projects. New Kotlin applications should call OpenRouter from trusted server-side code, a backend API, or another secure boundary. Do not embed the OpenRouter key in Android or desktop client binaries.

The previous GrowFoundry Kotlin AI SDK methods are deprecated compatibility wrappers. Use the GrowFoundry SDK for database, auth, storage, functions, and realtime; use OpenRouter for model calls.
