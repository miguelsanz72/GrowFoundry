...
description: File upload, download, and management with the GrowFoundry Kotlin SDK
---
...
```kotlin
val bucket = growfoundry.storage.from("images")
```
...
val result = growfoundry.storage
    .from("images")
...
val result = growfoundry.storage["images"]
...
val posts = growfoundry.database
    .from("posts")
    .select("image_key")
    .eq("id", "post-123")
...
```kotlin
val strategy = growfoundry.storage
    .from("images")
...
```kotlin
val buckets = growfoundry.storage.listBuckets()
...
```kotlin
val bucket = growfoundry.storage.createBucket("my-bucket") {
    isPublic = true
}
...
```kotlin
growfoundry.storage.updateBucket("my-bucket") {
    isPublic = false
}
```
...
```kotlin
growfoundry.storage.deleteBucket("my-bucket")
```
