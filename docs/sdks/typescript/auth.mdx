...
description: User authentication and profile management with the GrowFoundry TypeScript SDK
---
...
<Note>
  When `requireEmailVerification` is true, `accessToken` will be null until the user verifies their email. GrowFoundry sends a verification email with either a link or a 6-digit code, based on your dashboard configuration (`verifyEmailMethod`). For code verification, implement a page that prompts the user to enter the code (see [verifyEmail()](#verifyemail)). For link verification, provide a `redirectTo` URL that should receive the browser after GrowFoundry verifies the token. Recommended: use your sign-in page as `redirectTo`, then show a success message and ask the user to sign in with their email and password.
</Note>

### Example

```javascript
const { data, error } = await growfoundry.auth.signUp({
...
```javascript
const { data, error } = await growfoundry.auth.signInWithPassword({
  email: 'user@example.com',
  password: 'secure_password123',
});
...
<Note>
  After OAuth redirect, SDK automatically detects the callback `growfoundry_code`, exchanges it for a
...
await growfoundry.auth.signInWithOAuth('google', {
...
const { data } = await growfoundry.auth.signInWithOAuth('google', {
  redirectTo: 'http://localhost:3000/dashboard',
...
<Note>
  ...
  or `scope`; GrowFoundry sets those values on the server and ignores colliding client-provided keys.
</Note>

<Note>
  Custom providers must be configured first in the GrowFoundry dashboard under `Auth Methods` with client credentials and an OIDC discovery URL.
</Note>
...
```javascript
const { error } = await growfoundry.auth.signOut();
```
...
```javascript
const { data, error } = await growfoundry.auth.getCurrentUser();
...
```javascript
const { data, error } = await growfoundry.auth.getProfile('usr_xyz789');
...
```javascript
const { data, error } = await growfoundry.auth.setProfile({
  name: 'JohnDev',
  bio: 'Full-stack developer',
  avatar_url: 'https://example.com/avatar.jpg',
...
Resend email verification when the previous OTP has expired or was not received. Uses the method configured in auth settings (`verifyEmailMethod`). When method is `code`, sends a 6-digit numeric code. When method is `link`, sends a browser verification link that goes through an GrowFoundry backend endpoint first.
...
```javascript
const { data, error } = await growfoundry.auth.resendVerificationEmail({
  email: 'user@example.com',
  redirectTo: 'http://localhost:3000/sign-in',
});
...
```javascript
const { data, error } = await growfoundry.auth.verifyEmail({
  email: 'user@example.com',
  otp: '123456',
});
...
Send password reset email using the method configured in auth settings (`resetPasswordMethod`). When method is `code`, sends a 6-digit numeric code for two-step flow. When method is `link`, sends a browser reset link that goes through an GrowFoundry backend endpoint first.
...
```javascript
const { data, error } = await growfoundry.auth.sendResetPasswordEmail({
  email: 'user@example.com',
...
```javascript
const { data, error } = await growfoundry.auth.exchangeResetPasswordToken({
  email: 'user@example.com',
  code: '123456',
});
...
if (data) {
  ...
  await growfoundry.auth.resetPassword({
...
- `token`: Present only when `growfoundry_status=ready`. Pass this value to `resetPassword({ otp })`.
...
Only render the reset-password form when `growfoundry_status=ready` and `token` is present.
...
const { data, error } = await growfoundry.auth.resetPassword({
  newPassword: 'newSecurePassword123',
