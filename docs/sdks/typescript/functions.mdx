...
description: Invoke serverless functions with the GrowFoundry TypeScript SDK
---
...
```javascript
const { data, error } = await growfoundry.functions.invoke('hello-world', {
...
```javascript
const { data, error } = await growfoundry.functions.invoke('get-stats', {
  method: 'GET'
})
...
```javascript
const { data, error } = await growfoundry.functions.invoke('api-endpoint', {
  method: 'PUT',
...
export default async function(req: Request): Promise<Response> {
  ...
  const client = createClient({
    baseUrl: Deno.env.get('GROWFOUNDRY_BASE_URL'),
