...
description: Type-safe database operations using the GrowFoundry TypeScript SDK
---
...
const { error } = await growfoundry.database
  .from('posts')
  .delete()
  .eq('id', postId)
```
...
const { data } = await growfoundry.database
  .from('products')
  .select()
...
const { data, count } = await growfoundry.database
  .from('posts')
  .select('*', { count: 'exact' })
  .range(from, to)
  .order('created_at', { ascending: false })
...
```tsx
import { useUser } from '@growfoundry/react'; // or '@growfoundry/react-router'
