...
description: File upload, download, and management with the GrowFoundry TypeScript SDK
---
...
```javascript
const bucket = growfoundry.storage.from('images')
```
...
```json
{
  "data": {
    ...
    "url": "https://your-app.region.growfoundry.app/api/storage/buckets/images/objects/posts%2Fpost-123%2Fcover.jpg"
  },
  "error": null
}
```
...
```json
{
  "data": {
    ...
    "mimeType": "image/jpeg",
    "uploadedAt": "2024-01-15T10:30:00Z",
    "url": "https://your-app.region.growfoundry.app/api/storage/buckets/uploads/objects/myfile-1705315200000-abc123.jpg"
  },
  "error": null
}
```
...
const { data: post, error: dbError } = await growfoundry.database
  .from('posts')
  .select('image_key')
  .eq('id', 'post-123')
  .single()
...
const { data: blob, error } = await growfoundry.storage
  .from('images')
