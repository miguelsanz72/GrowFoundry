---
title: Model Gateway
description: Build AI features with the OpenAI SDK and the OpenRouter key provisioned by GrowFoundry
---

## Overview

GrowFoundry provisions an OpenRouter API key for Model Gateway projects. For new AI features, use that key directly with OpenRouter's OpenAI-compatible API instead of the deprecated GrowFoundry AI proxy methods.

Use the GrowFoundry dashboard to copy the active OpenRouter key, store it in `OPENROUTER_API_KEY`, and call OpenRouter from trusted server-side code.
...
## Using GrowFoundry Data with AI

Use the GrowFoundry SDK for database, auth, storage, functions, and realtime. Use OpenRouter directly for model calls.
...
const growfoundry = createClient({
  baseUrl: process.env.GROWFOUNDRY_BASE_URL!,
  anonKey: process.env.GROWFOUNDRY_ANON_KEY!,
});
...
const { data: documents } = await growfoundry.database
  .from('documents')
  .select()
  .limit(5);
...
## Legacy GrowFoundry AI Proxy

The previous GrowFoundry AI SDK methods are deprecated:

- `growfoundry.ai.chat.completions.create()`
- `growfoundry.ai.images.generate()`
- `growfoundry.ai.embeddings.create()`

They map to backend proxy routes that still exist for compatibility, but new integrations should use OpenRouter directly. The direct path gives access to OpenRouter's complete and evolving API surface without waiting for GrowFoundry SDK wrapper updates.

## Model Catalog

Use the GrowFoundry dashboard or OpenRouter model catalog to choose a model. Model IDs use the `provider/model` format, such as `openai/gpt-4o`, `anthropic/claude-3.5-haiku`, and `google/gemini-2.5-flash-image`.
