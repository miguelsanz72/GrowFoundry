...
The Payments SDK exposes runtime methods for generated app frontends:

- `insforge.payments.createCheckoutSession(...)`
- `insforge.payments.createCustomerPortalSession(...)`

Stripe secret keys, catalog management, webhook setup, and payment monitoring in the InsForge dashboard are admin operations. Configure them from the Dashboard or CLI before using the SDK.
...
<Note>
  See [Payments overview](/core-concepts/payments/overview) for setup, webhook projections, and fulfillment patterns.
</Note>

The SDK methods require the current InsForge user token. Anonymous InsForge tokens can be used for guest one-time checkout, but an InsForge API key is not a substitute for these runtime calls because the backend needs a user context for local session rows.
...
Customer portal sessions require an authenticated user and an existing customer mapping for the subject. The mapping is usually created after a Checkout Session completes and Stripe returns a customer. The synced customer data shown in the Payments dashboard can help you verify what happened, but portal creation relies on `payments.customer_mappings`.
...
In practice, one-time checkout can support guest or signed-in flows when the client has an InsForge user token. Customer portal creation requires an authenticated user because it operates on an existing billing subject mapping.

The backend does not infer membership from arbitrary `subject` values. If users can pass shared subjects such as teams or organizations, add an authorization boundary before exposing subscription checkout or customer portal UI. For example, enable RLS on `payments.stripe_checkout_sessions` and `payments.stripe_customer_portal_sessions` with policies that check team membership, or call Payments through your own server endpoint that checks membership first.
...
The Payments SDK does not expose generic end-user reads for `payments.customers`, provider subscription tables, or provider payment activity tables such as `payments.stripe_payment_activity` and `payments.razorpay_payment_activity`. Those tables are operational records used for dashboard visibility and fulfillment workflows.
...
Populate those tables from webhook-projected payment state. See [Payments overview](/core-concepts/payments/overview) for the fulfillment model.
...
Then add a database trigger that listens to the relevant provider activity table, such as `payments.stripe_payment_activity`, and updates `public.orders` when the matching row becomes `type = 'one_time_payment'` and `status = 'succeeded'`. If the app accepts multiple payment providers, reuse one trigger function and attach it to each provider activity table. The success page can read the order and show `pending`, `paid`, or `fulfilled`.
...
<CardGroup cols={2}>
  ...
  <Card title="Protect Billing Subjects" icon="shield-check">
    Add RLS or server-side membership checks before exposing subscription or portal flows.
  </Card>
</CardGroup>
