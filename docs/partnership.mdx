...
description: Learn how to integrate GrowFoundry as a partner platform through co-branded or white-label partnerships
---
...
sequenceDiagram
    ...
    participant GrowFoundry as GrowFoundry API
    participant Dashboard as GrowFoundry Dashboard

    User->>Partner: Sign up/Login
    Partner->>GrowFoundry: POST /connect-user<br/>(name, email)
    GrowFoundry-->>Partner: Return user account ID

    User->>Partner: Create new project
    Partner->>GrowFoundry: POST /sync-project<br/>(userId, project_name)

    alt Success
        GrowFoundry-->>Partner: Return project details<br/>(id, access_host, api_key)
        Partner->>Partner: Store project credentials
        Partner->>User: Show project ready
    else Project Limit Reached
        GrowFoundry-->>Partner: Return error + candidate projects
        Partner->>User: Show existing projects to choose
        User->>Partner: Select existing project
        Partner->>GrowFoundry: GET /metadata<br/>(projectId)
        GrowFoundry-->>Partner: Return project details
    end

    User->>Partner: Use application features
    Partner->>GrowFoundry: API calls using project credentials
    GrowFoundry-->>Partner: Return data
    Partner-->>User: Display results

    Note over User,Dashboard: User can also access GrowFoundry Dashboard directly
...
sequenceDiagram
    ...
    Note over User,GrowFoundry: User never interacts with GrowFoundry directly

    User->>Partner: Sign up/Login to partner platform

    User->>Partner: Create new project
    Partner->>GrowFoundry: POST /sync-embedded-project<br/>(project_name, region, instance_type)
    GrowFoundry-->>Partner: Return project details<br/>(id, access_host, api_key, status)
    Partner->>Partner: Store project credentials
    Partner->>User: Show project ready
    ...
    Partner->>GrowFoundry: POST /pause<br/>(projectId)
    GrowFoundry-->>Partner: Return status: paused
    Partner->>User: Confirm project paused

    User->>Partner: Request to restore project
    Partner->>GrowFoundry: POST /restore<br/>(projectId)
    GrowFoundry-->>Partner: Return status: active
    Partner->>User: Confirm project active

    Partner->>GrowFoundry: GET /metadata<br/>(projectId)
    GrowFoundry-->>Partner: Return current project status
```
