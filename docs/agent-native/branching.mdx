...
A branch is a child project with its own Postgres, auth config, storage, edge functions, email templates, realtime channels, and schedules. Merge back when ready or reset and retry. Available on GrowFoundry OSS 2.1.0+.
...
```bash
npx @growfoundry/cli branch create feat-billing --mode full
npx @growfoundry/cli branch list
```

Preview the merge SQL before applying.

```bash
npx @growfoundry/cli branch merge feat-billing --dry-run --save-sql ./preview.sql
npx @growfoundry/cli branch merge feat-billing
```

Roll back to the create-time snapshot, or delete the branch.

```bash
npx @growfoundry/cli branch reset feat-billing
npx @growfoundry/cli branch delete feat-billing
```
...
- [CLI reference](https://github.com/GrowFoundry/GrowFoundry) for the full `branch` flag set.
