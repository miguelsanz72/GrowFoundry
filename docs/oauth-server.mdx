---
title: OAuth Server
description: Use GrowFoundry as an OAuth 2.0 identity provider to authenticate users in third-party applications
---

## Overview

GrowFoundry can function as an OAuth 2.0 identity provider, allowing third-party applications to authenticate users with "Sign in with GrowFoundry". This enables developers building on your platform to leverage GrowFoundry's authentication system without managing their own user credentials.

## Use Cases

<CardGroup cols={2}>
  <Card title="Developer Platforms" icon="code">
    Enable third-party developers to build integrations with "Sign in with GrowFoundry" while you maintain control over user data access.
  </Card>
  ...
  <Card title="Partner Applications" icon="handshake">
    Allow partner applications to authenticate users against your GrowFoundry project without sharing credentials.
  </Card>
...
GrowFoundry implements the **Authorization Code flow with PKCE** (Proof Key for Code Exchange), the most secure OAuth flow for both web and native applications.

```mermaid
sequenceDiagram
    participant App as Your Application
    participant User as User Browser
    participant GrowFoundry as GrowFoundry Auth
    participant API as GrowFoundry API

    App->>App: Generate code_verifier & code_challenge
    App->>User: Redirect to /api/oauth/v1/authorize
    User->>GrowFoundry: User authenticates
    GrowFoundry->>User: Authorization prompt
    User->>GrowFoundry: User approves
    GrowFoundry->>App: Redirect with authorization code
    App->>GrowFoundry: POST /api/oauth/v1/token<br/>(code + code_verifier)
    GrowFoundry->>GrowFoundry: Verify PKCE
    GrowFoundry->>App: Access token + Refresh token
...
<Steps>
  <Step title="Register Your Application">
    Contact GrowFoundry to register your application as an OAuth client. You'll receive:
...
```
GET https://api.growfoundry.dev/api/oauth/v1/authorize
```
...
```
https://api.growfoundry.dev/api/oauth/v1/authorize?
...
```
POST https://api.growfoundry.dev/api/oauth/v1/token
```
...
```
GET https://api.growfoundry.dev/auth/v1/profile
...
GROWFOUNDRY_CLIENT_ID=clf_your_client_id
GROWFOUNDRY_CLIENT_SECRET=your_client_secret
GROWFOUNDRY_URL=https://api.growfoundry.dev
...
const requiredEnvVars = ['SESSION_SECRET', 'GROWFOUNDRY_CLIENT_ID', 'GROWFOUNDRY_CLIENT_SECRET'];
...
const config = {
  clientId: process.env.GROWFOUNDRY_CLIENT_ID,
  clientSecret: process.env.GROWFOUNDRY_CLIENT_SECRET,
  growfoundryUrl: process.env.GROWFOUNDRY_URL || 'https://api.growfoundry.dev',
...
app.get('/auth/login', (req, res) => {
  ...
  const authUrl = new URL(`${config.growfoundryUrl}/api/oauth/v1/authorize`);
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    const tokenResponse = await fetch(`${config.growfoundryUrl}/api/oauth/v1/token`, {
      method: 'POST',
    ...
    // Fetch user profile
    const profileResponse = await fetch(`${config.growfoundryUrl}/auth/v1/profile`, {
...
app.get('/api/organizations', async (req, res) => {
  ...
  const response = await fetch(`${config.growfoundryUrl}/organizations/v1`, {
...
| `iss` | Issuer (`growfoundry`) |
| `aud` | Audience (`growfoundry-api`) |
...
<Card title="OAuth Example Repository" icon="github" href="https://github.com/GrowFoundry/growfoundry-oauth-example">
  Complete working example showing how to integrate "Sign in with GrowFoundry" into your application.
</Card>
