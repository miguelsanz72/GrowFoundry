# Growfoundry OSS Authentication API Documentation

## Overview

Growfoundry uses JWT tokens and API keys for authentication. Store tokens in localStorage after login.
...
Growfoundry supports Google and GitHub OAuth when configured with environment variables.
...
2. Configure each platform's Client ID/Client Secret in GrowFoundry backend (via Environment Variables)
