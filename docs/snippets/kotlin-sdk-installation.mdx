1. Add GrowFoundry dependencies to your project
...
<Tabs>
  <Tab title="Maven Central">
    ...
    dependencies {
        implementation("dev.growfoundry:growfoundry-kotlin:0.1.6")
    }
    ```
  </Tab>
    ...
    <AccordionGroup>
      <Accordion title="Option A: Environment Variables">
        ...
        repositories {
            mavenCentral()
            maven {
                url = uri("https://maven.pkg.github.com/GrowFoundry/growfoundry-kotlin")
                credentials {
...
2. Initialize GrowFoundry SDK

```kotlin
import dev.growfoundry.createGrowfoundryClient
import dev.growfoundry.auth.Auth
import dev.growfoundry.database.Database
import dev.growfoundry.storage.Storage
import dev.growfoundry.functions.Functions
import dev.growfoundry.realtime.Realtime
import dev.growfoundry.ai.AI

val client = createGrowfoundryClient(
...
```kotlin
import dev.growfoundry.GrowfoundryLogLevel
...
val client = createGrowfoundryClient(
    ...
    logLevel = GrowfoundryLogLevel.DEBUG
...
2. Initialize GrowFoundry SDK (With Chrome Custom Tabs and Session Storage)
...
import dev.growfoundry.ai.AI
import dev.growfoundry.auth.Auth
import dev.growfoundry.auth.BrowserLauncher
import dev.growfoundry.auth.ClientType
import dev.growfoundry.auth.SessionStorage
import dev.growfoundry.database.Database
import dev.growfoundry.functions.Functions
import dev.growfoundry.realtime.Realtime
import dev.growfoundry.storage.Storage

class GrowfoundryManager(private val context: Context) {
...
val Context.authDataStore: DataStore<Preferences> by preferencesDataStore(name = "growfoundry_auth")
...
// Then use it in your GrowFoundry client
install(Auth) {
    browserLauncher = ...
    persistSession = true
