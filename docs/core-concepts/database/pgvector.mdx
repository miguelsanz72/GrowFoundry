...
[pgvector](https://github.com/pgvector/pgvector) ships on every GrowFoundry project. Use it for semantic search, recommendations, and [RAG](https://www.pinecone.io/learn/retrieval-augmented-generation/).
...
const growfoundry = createClient({ projectId: process.env.GROWFOUNDRY_PROJECT_ID });
...
await growfoundry.database.from('documents').insert({
  content: 'hello world',
...
- [Model Gateway overview](/core-concepts/ai/overview) for the GrowFoundry side of OpenRouter.
