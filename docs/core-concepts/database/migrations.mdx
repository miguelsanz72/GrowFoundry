...
description: "Track schema changes in git and apply them with the GrowFoundry CLI"
---

Migrations are versioned SQL files in `migrations/` applied with `@growfoundry/cli`. Each successful run is recorded in `system.custom_migrations`. The workflow is forward-only.
...
npx @growfoundry/cli db migrations new create-employees-table
```
...
npx @growfoundry/cli db migrations list
```
