...
description: Use any AWS SigV4 client against GrowFoundry Storage
---

GrowFoundry Storage speaks the [AWS S3 protocol](https://docs.aws.amazon.com/AmazonS3/latest/API/Welcome.html) at `/storage/v1/s3`. Cloud projects only.
...
[profile growfoundry]
region = us-east-2
endpoint_url = https://project_ref.region.growfoundry.app/storage/v1/s3
s3 =
  addressing_style = path
```

```bash
aws --profile growfoundry s3 cp ./photo.jpg s3://my-bucket/photo.jpg
aws --profile growfoundry s3 sync ./dist s3://my-bucket/dist
```
...
const client = new S3Client({
  forcePathStyle: true,
  region: 'us-east-2',
  endpoint: 'https://project_ref.region.growfoundry.app/storage/v1/s3',
