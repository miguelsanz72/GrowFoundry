...
Provider is resolved on every send, so saves take effect on the next request. GrowFoundry runs `transporter.verify()` before saving, so a persisted config always works. Passwords are encrypted at rest with AES-256-GCM and never returned by the API.
...
<Steps>
  ...
  <Step title="Save">
    GrowFoundry runs an SMTP handshake before persisting. Bad credentials fail fast.
  </Step>
