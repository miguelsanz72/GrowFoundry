...
description: "Call any LLM through one GrowFoundry-managed key, with per-project quotas."
---
...
<Frame caption="One OpenAI-compatible endpoint with per-provider access, ready-to-copy code, and usage tracking.">
  <img src="/images/model-gateway-overview.png" alt="GrowFoundry dashboard Model Gateway overview showing code samples, provider chips, and usage charts" />
</Frame>
...
```mermaid
graph TB
    Dashboard[GrowFoundry Dashboard] --> Key[Active OpenRouter Key]
