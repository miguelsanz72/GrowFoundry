...
| --- | --- | --- | --- |
| Pure helper, parser, formatter, state reducer | Unit | `packages/dashboard/src/**/__tests__/*.test.ts` | `npm --workspace @growfoundry/dashboard run test:unit` |
| React component behavior, forms, dialogs, conditional rendering | Component | `packages/dashboard/src/**/__tests__/*.test.tsx` | `npm --workspace @growfoundry/dashboard run test:component` |
| Routing, auth redirects, host-mode integration, browser-only behavior | UI smoke | `packages/dashboard/tests/ui/*.spec.ts` | `npm --workspace @growfoundry/dashboard run test:ui` |

Conventions:

- `npm --workspace @growfoundry/dashboard run test` runs the full Vitest suite: unit plus component.
...
2. `packages/dashboard/src/lib/utils/utils.ts` — `isGrowFoundryCloudProject()` → `return true;` (was the `.growfoundry.app` hostname check).
