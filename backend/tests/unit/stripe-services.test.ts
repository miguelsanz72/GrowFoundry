...
function expectStripeScopedDelete(
  ...
  expect(queryMock).toHaveBeenCalledWith(
    expect.stringMatching(
      new RegExp(
        `DELETE FROM payments\\.${tableName}[\\s\\S]*provider\\s*=\\s*(?:'stripe'|\\$2)`,
        'i'
      )
    ),
    [environment, 'stripe']
...
function expectStripeRuntimeDelete(
  queryMock: typeof mockPool.query,
  tableName: string,
  environment: string
) {
  expect(queryMock).toHaveBeenCalledWith(
    expect.stringMatching(
      new RegExp(`DELETE FROM payments\\.${tableName}[\\s\\S]*environment\\s*=\\s*\\$1`, 'i')
    ),
    [environment]
  );
}
...
import { StripeSyncService } from '../../src/services/payments/stripe/sync.service';
import { StripeWebhookService } from '../../src/services/payments/stripe/webhook.service';

const stripeServices = {
  getConfig: () => StripeConfigService.getInstance().getConfig(),
  setStripeSecretKey: (environment: 'test' | 'live', secretKey: string) =>
    StripeConfigService.getInstance().setStripeSecretKey(
      environment,
      secretKey,
      async (syncEnvironment, provider) => {
        await StripeSyncService.getInstance().syncPaymentsEnvironmentAfterKeyChange(
          syncEnvironment,
          provider
        );
      }
    ),
  removeStripeSecretKey: (environment: 'test' | 'live') =>
    StripeConfigService.getInstance().removeStripeSecretKey(environment),
  seedStripeKeysFromEnv: () => StripeSyncService.getInstance().seedStripeKeysFromEnv(),
  getStatus: () => StripeConfigService.getInstance().getStatus(),
  configureWebhook: async (environment: 'test' | 'live') => ({
    connection: await StripeConfigService.getInstance().configureManagedStripeWebhook(environment),
  }),
  syncPayments: (input: { environment: 'test' | 'live' | 'all' }) =>
    StripeSyncService.getInstance().syncPayments(input),
  listProducts: (input: { environment: 'test' | 'live' }) =>
    StripeProductService.getInstance().listProducts(input),
  getProduct: (environment: 'test' | 'live', productId: string) =>
    StripeProductService.getInstance().getProduct(environment, productId),
  createProduct: (input: Parameters<StripeProductService['createProduct']>[0]) =>
    StripeProductService.getInstance().createProduct(input),
  updateProduct: (productId: string, input: Parameters<StripeProductService['updateProduct']>[1]) =>
    StripeProductService.getInstance().updateProduct(productId, input),
  deleteProduct: (environment: 'test' | 'live', productId: string) =>
    StripeProductService.getInstance().deleteProduct(environment, productId),
  listPrices: (input: Parameters<StripePriceService['listPrices']>[0]) =>
    StripePriceService.getInstance().listPrices(input),
  getPrice: (environment: 'test' | 'live', priceId: string) =>
    StripePriceService.getInstance().getPrice(environment, priceId),
  createPrice: (input: Parameters<StripePriceService['createPrice']>[0]) =>
    StripePriceService.getInstance().createPrice(input),
  updatePrice: (priceId: string, input: Parameters<StripePriceService['updatePrice']>[1]) =>
    StripePriceService.getInstance().updatePrice(priceId, input),
  archivePrice: (environment: 'test' | 'live', priceId: string) =>
    StripePriceService.getInstance().archivePrice(environment, priceId),
  createCheckoutSession: (
    input: Parameters<StripeCheckoutService['createCheckoutSession']>[0],
    user: Parameters<StripeCheckoutService['createCheckoutSession']>[1]
  ) => StripeCheckoutService.getInstance().createCheckoutSession(input, user),
  createCustomerPortalSession: (
    input: Parameters<StripeCustomerPortalService['createCustomerPortalSession']>[0],
    user: Parameters<StripeCustomerPortalService['createCustomerPortalSession']>[1]
  ...
  listSubscriptions: (input: Parameters<StripeSubscriptionService['listSubscriptions']>[0]) =>
    StripeSubscriptionService.getInstance().listSubscriptions(input),
};
...
describe('Stripe payment services', () => {
  ...
  it('reports Stripe key configuration from the secret store', async () => {
    ...
    await expect(stripeServices.getConfig()).resolves.toEqual({
      keys: [
        {
          environment: 'test',
          hasKey: true,
  ...
  it('upserts encrypted Stripe keys into the canonical secret names and syncs payments immediately', async () => {
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [{ accountId: 'acct_123' }],
      })
      .mockResolvedValueOnce({ rows: [connectedTestConnectionRow] })
      .mockResolvedValueOnce({ rows: [connectedTestConnectionRow] });

    await stripeServices.setStripeSecretKey('test', ' sk_test_newsecret1234 ');
    ...
    expect(mockClient.query).not.toHaveBeenCalledWith(
      'DELETE FROM payments.stripe_subscription_items WHERE environment = $1',
      ['test']
    );
    expect(mockClient.query).not.toHaveBeenCalledWith(
      'DELETE FROM payments.stripe_subscriptions WHERE environment = $1',
      ['test']
    );
    expect(mockClient.query).not.toHaveBeenCalledWith(
      'DELETE FROM payments.stripe_products WHERE environment = $1',
      ['test']
    );
  ...
  it('saves Stripe keys even when managed webhook setup fails', async () => {
    ...
    await expect(
      stripeServices.setStripeSecretKey('test', 'sk_test_newsecret1234')
  ...
  it('skips webhook recreation and sync when saving the same Stripe key', async () => {
    ...
    await stripeServices.setStripeSecretKey('test', ' sk_test_1234567890 ');
  ...
  it('configures managed Stripe webhooks on demand using the stored key', async () => {
    ...
    const result = await stripeServices.configureWebhook('test');
  ...
  it('clears the payment mirror when on-demand webhook configuration finds another Stripe account', async () => {
    ...
    expectStripeRuntimeDelete(mockClient.query, 'stripe_prices', 'test');
    expectStripeRuntimeDelete(mockClient.query, 'stripe_products', 'test');
    expect(result.connection.accountId).toBe('acct_new');
  });
  ...
  it('requires a configured Stripe key before configuring managed webhooks', async () => {
    ...
    await expect(stripeServices.configureWebhook('test')).rejects.toThrow(
  ...
  it('persists rotated keys without recreating webhooks or syncing when the Stripe account is unchanged', async () => {
    ...
    await stripeServices.setStripeSecretKey('test', 'sk_test_rotated1234');
  ...
  it('soft-removes Stripe keys without clearing the mirrored payment data', async () => {
    ...
    await expect(stripeServices.removeStripeSecretKey('live')).resolves.toBe(true);
    ...
    expect(mockClient.query).not.toHaveBeenCalledWith(
      'DELETE FROM payments.stripe_prices WHERE environment = $1',
      ['live']
    );
    expect(mockClient.query).not.toHaveBeenCalledWith(
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/UPDATE payments\.provider_connections/i),
  ...
  it('seeds Stripe keys from environment variables', async () => {
    ...
    try {
      await stripeServices.seedStripeKeysFromEnv();
    } finally {
  ...
  it('fetches Stripe products and prices and commits a successful sync', async () => {
    ...
    const result = await stripeServices.syncPayments({ environment: 'test' });
    ...
    expect(mockSyncClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/DELETE FROM payments\.stripe_prices/i),
      ['test', ['price_123']]
    );
    expect(mockSyncClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/DELETE FROM payments\.stripe_products/i),
      ['test', ['prod_123']]
    );
  ...
  it('syncs payments by refreshing catalog before subscriptions', async () => {
    ...
    expect(result.results[0]).toMatchObject({
      environment: 'test',
      connection: { status: 'connected', accountId: 'acct_123' },
    ...
    expect(mockSubscriptionsClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/DELETE FROM payments\.stripe_subscriptions/i),
      ['test', []]
    );
  });
  ...
  it('continues syncing subscriptions when customer mirroring fails', async () => {
    ...
    await expect(stripeServices.syncPayments({ environment: 'test' })).resolves.toMatchObject({
      results: [
        {
          environment: 'test',
          connection: { status: 'connected', accountId: 'acct_123' },
  ...
  it('clears account-scoped payment mirrors when catalog sync resolves to a different account', async () => {
    ...
    expectStripeScopedDelete(mockSyncClient.query, 'customer_mappings', 'test');
    expectStripeScopedDelete(mockSyncClient.query, 'webhook_events', 'test');
    expectStripeRuntimeDelete(mockSyncClient.query, 'stripe_prices', 'test');
    expectStripeRuntimeDelete(mockSyncClient.query, 'stripe_products', 'test');
    expect(mockProvider.createWebhookEndpoint).toHaveBeenCalledTimes(1);
    expect(mockEncrypt).toHaveBeenCalledWith('whsec_new');
  ...
  it('lists products from the requested local Stripe mirror environment', async () => {
    ...
    await expect(stripeServices.listProducts({ environment: 'test' })).resolves.toEqual({
      products: [
        {
          environment: 'test',
          productId: 'prod_123',
    ...
    expect(mockPool.query).toHaveBeenCalledWith(
      expect.stringMatching(/FROM payments\.stripe_products/i),
      ['test']
    );
  });
  ...
  it('creates products with the requested Stripe key and updates only the returned product mirror', async () => {
    ...
    const result = await stripeServices.createProduct({
      environment: 'live',
      name: 'New Product',
      active: true,
      metadata: { tier: 'new' },
    ...
    expect(mockMirrorClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/INSERT INTO payments\.stripe_products/i),
  ...
  it('updates and deletes products through the requested Stripe provider', async () => {
    ...
    await expect(
      stripeServices.updateProduct('prod_123', {
        environment: 'test',
        name: 'Updated Product',
        active: false,
      })
    ...
    await expect(stripeServices.deleteProduct('test', 'prod_123')).resolves.toEqual({
      productId: 'prod_123',
      deleted: true,
    });
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/DELETE FROM payments\.stripe_prices[\s\S]*product_id = \$2/i),
      ['test', 'prod_123']
    );
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/DELETE FROM payments\.stripe_products[\s\S]*product_id = \$2/i),
      ['test', 'prod_123']
    );
    expect(mockProvider.syncCatalog).not.toHaveBeenCalled();
  });
  ...
  it('creates, updates, and archives prices through the requested Stripe provider', async () => {
    ...
    await expect(
      stripeServices.updatePrice('price_123', {
        environment: 'test',
        active: false,
        metadata: { archived: 'true' },
      })
    ...
    await expect(stripeServices.archivePrice('test', 'price_123')).resolves.toMatchObject({
      price: {
        environment: 'test',
        priceId: 'price_123',
        active: false,
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/INSERT INTO payments\.stripe_prices/i),
  ...
  it('uses a shared environment lock and a key-scoped lock for idempotent checkout creation', async () => {
    ...
    await stripeServices.createCheckoutSession(
      {
        environment: 'test',
        mode: 'subscription',
        lineItems: [{ priceId: 'price_123', quantity: 1 }],
  ...
  it('creates an authorized checkout row before identified Stripe checkout', async () => {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/INSERT INTO payments\.stripe_checkout_sessions/i),
  ...
  it('returns an existing checkout session for matching caller idempotency retries', async () => {
    const existingCheckoutSessionRow = {
      ...
      checkoutSessionId: 'cs_existing_123',
    ...
    const mockClient = {
      query: vi.fn().mockImplementation((sql: string) => {
        ...
        if (/FROM payments\.stripe_checkout_sessions[\s\S]*idempotency_key = \$2/i.test(sql)) {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/FROM payments\.stripe_checkout_sessions[\s\S]*idempotency_key = \$2/i),
      [
        'test',
        'checkout-123',
        'subscription',
  ...
  it('resumes incomplete idempotent checkout rows instead of returning unusable rows', async () => {
    ...
    mockPool.query.mockImplementation((sql: string) => {
      if (/SELECT customer_id AS "providerCustomerId"/i.test(sql)) {
        return Promise.resolve({ rows: [] });
      }

      if (/UPDATE payments\.stripe_checkout_sessions/i.test(sql)) {
  ...
  it('reuses an existing Stripe customer mapping for identified checkout', async () => {
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [{ providerCustomerId: 'cus_existing' }],
      })
      .mockResolvedValueOnce({
        rows: [
          {
  ...
  it('does not call Stripe when stripe_checkout_sessions RLS denies the insert', async () => {
    const mockClient = {
      query: vi.fn().mockImplementation((sql: string) => {
        if (/INSERT INTO payments\.stripe_checkout_sessions/i.test(sql)) {
  ...
  it('creates a Stripe customer portal session from an existing billing subject mapping', async () => {
    const mockClient = {
      query: vi.fn().mockImplementation((sql: string) => {
        if (/INSERT INTO payments\.stripe_customer_portal_sessions/i.test(sql)) {
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [{ providerCustomerId: 'cus_123' }],
      })
      .mockResolvedValueOnce({
        rows: [
          {
    ...
    await expect(
      stripeServices.createCustomerPortalSession(
        {
          environment: 'test',
          subject: { type: 'team', id: 'team_123' },
          returnUrl: 'https://example.com/account',
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(/INSERT INTO payments\.stripe_customer_portal_sessions/i),
  ...
  it('rejects customer portal sessions when the billing subject has no Stripe customer mapping', async () => {
    ...
    expect(mockPool.query).toHaveBeenCalledWith(
      expect.stringMatching(/UPDATE payments\.stripe_customer_portal_sessions/i),
  ...
  it('does not call Stripe when stripe_customer_portal_sessions RLS denies the insert', async () => {
    const mockClient = {
      query: vi.fn().mockImplementation((sql: string) => {
        if (/INSERT INTO payments\.stripe_customer_portal_sessions/i.test(sql)) {
          return Promise.reject({ code: '42501', message: 'new row violates row-level security' });
    ...
    ).rejects.toThrow(/stripe_customer_portal_sessions RLS policies/i);

    expect(mockProvider.createCustomerPortalSession).not.toHaveBeenCalled();
    expect(mockPool.query).not.toHaveBeenCalledWith(
      expect.stringMatching(/customer_mappings/i),
      expect.any(Array)
    );
  });
  ...
  it('stores duplicate processed Stripe webhook events without reprocessing', async () => {
    ...
    await expect(
      stripeServices.handleStripeWebhook('test', Buffer.from('{"id":"evt_123"}'), 'sig_123')
    ).resolves.toMatchObject({
      received: true,
      handled: false,
      event: {
        ...
        rows: [
          {
            ...
            attemptCount: 1,
            lastError: 'activity write failed',
            receivedAt: new Date('2026-04-28T00:00:00.000Z'),
            processedAt: null,
            createdAt: new Date('2026-04-28T00:00:00.000Z'),
            updatedAt: new Date('2026-04-28T00:00:01.000Z'),
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [
          {
            environment: 'test',
            eventId: 'evt_delayed_completed_123',
            eventType: 'checkout.session.completed',
            livemode: false,
            accountId: null,
            objectType: 'checkout.session',
  ...
  it('deduplicates one-time checkout activity by checkout session when PaymentIntent is absent', async () => {
    mockGetSecretByKey
      .mockResolvedValueOnce('whsec_test_123')
      .mockResolvedValueOnce('sk_test_1234567890');
    mockProvider.constructWebhookEvent.mockReturnValueOnce({
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [
          {
            environment: 'test',
            eventId: 'evt_no_payment_required_123',
            eventType: 'checkout.session.completed',
            livemode: false,
            accountId: null,
            objectType: 'checkout.session',
  ...
  it('records subscription invoice payment activity from invoice webhooks', async () => {
    ...
    await expect(
      stripeServices.handleStripeWebhook('test', Buffer.from('{"id":"evt_invoice_123"}'), 'sig_123')
    ).resolves.toMatchObject({ received: true, handled: true });
  ...
  it('records refund activity and copies context from the original payment', async () => {
    ...
    await expect(
      stripeServices.handleStripeWebhook('test', Buffer.from('{"id":"evt_refund_123"}'), 'sig_123')
    ).resolves.toMatchObject({ received: true, handled: true });
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [
          {
            environment: 'test',
            eventId: 'evt_refund_sub_early_123',
            eventType: 'refund.created',
            livemode: false,
            accountId: null,
            objectType: 'refund',
  ...
  it('upserts subscription projections from subscription webhooks', async () => {
    ...
    await expect(
      stripeServices.handleStripeWebhook('test', Buffer.from('{"id":"evt_sub_123"}'), 'sig_123')
    ).resolves.toMatchObject({ received: true, handled: true });
  ...
  it('syncs existing Stripe subscriptions as unmapped when no billing subject exists', async () => {
    ...
    const subscriptionInsertCall = mockClient.query.mock.calls.find(([sql]) =>
      /INSERT INTO payments\.stripe_subscriptions/i.test(String(sql))
    );
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(
        /DELETE FROM payments\.stripe_subscription_items[\s\S]*NOT \(subscription_item_id = ANY/i
      ),
      ['test', 'sub_existing', ['si_existing', 'si_extra']]
    );
    expect(mockClient.query).toHaveBeenCalledWith(
      expect.stringMatching(
        /DELETE FROM payments\.stripe_subscriptions[\s\S]*NOT \(subscription_id = ANY/i
      ),
      ['test', ['sub_existing']]
    );
  });
      ...
      rows: [
        {
          ...
          failedAt: null,
          refundedAt: null,
          providerCreatedAt: new Date('2026-04-28T00:00:00.000Z'),
      ...
      paymentActivity: [
        {
          ...
          failedAt: null,
          refundedAt: null,
          providerCreatedAt: '2026-04-28T00:00:00.000Z',
  ...
  it('lists subscriptions with their subscription items', async () => {
    ...
    await expect(
      stripeServices.listSubscriptions({
        environment: 'test',
        subjectType: 'organization',
        subjectId: 'org_123',
        limit: 10,
  ...
  it('clears Stripe customer mappings when customer.deleted webhooks arrive', async () => {
    ...
    mockPool.query
      .mockResolvedValueOnce({
        rows: [
          {
            environment: 'test',
            eventId: 'evt_customer_deleted_123',
    ...
    expect(mockPool.query).toHaveBeenCalledWith(
      expect.stringMatching(/DELETE FROM payments\.customer_mappings/i),
